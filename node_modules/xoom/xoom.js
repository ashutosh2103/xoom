...
const setAdjecentelements = (data, startNode, key) => {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        //clone.innerText = data[i];
        var _sihtml = startNode.innerHTML;
        clone.removeAttribute('x-loop');
        if (typeof (data[i]) === "object") {
            for (const subkey in data[i]) {
                if (data[i].hasOwnProperty(subkey)) {
                    _sihtml = _sihtml.replace(new RegExp(escapeRegExp("{" + key + "/" + subkey + "}"), 'gi'), data[i][subkey]);
                }
            }
